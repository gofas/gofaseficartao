...
add_hook('AdminInvoicesControlsOutput', 1, function($vars) {
    ...
    if ($vars['paymentmethod'] == 'gofaseficartao') {
		...
		$htmlOutput .= '<input style="display: none;" type="tel" name="installmentsnum" id="installmentsnum" value="1" class="form-control" autocomplete="off"/>';
		
		// <input type="tel" id="cardcvv" name="cardcvv" class="form-control" autocomplete="off" maxlength="4" placeholder="123 (Opcional)">
		...
		if($params['installments'] and ( (float)$minimunamountinstallments <= (float)$vars['total']) ){
			$htmlOutput .= '<input type="hidden" name="installment_" id="installment_" class="form-control" value="yes" />';
			$htmlOutput .= '<script>sessionStorage.setItem("installment_", "yes");</script>';
			$options_installments .= '<label style=margin-top:10px; class=col-sm-4 control-label>Parcelamento</label>';
			$options_installments .= '<div class=col-sm-6 style=margin: 0px 0px 20px 0px;max-width:100%;>';
			$options_installments .= '<select id=installmentsSelect name=installmentsSelect style=max-width: 320px; width: 320px; required= class=form-control>';
			$options_installments .= '<option value=1>Selecione o cartão para ver as opções</option>'; 
			$options_installments .= '</select></div>';
		}
		else{
			$htmlOutput .= '<input type="hidden" name="installment_" id="installment_" value="no" />';
		...
		//if(!empty($vars_->clientsdetails->cctype)){
			$htmlOutput .= '<script type="text/javascript">
				document.getElementById("btnShowAttemptCaptureDialog").addEventListener("click", gefic_on_load);
				function gefic_on_load(){
					if(document.getElementById("installment_").value == "yes"){
						var options_installments = "'.$options_installments.'";	
						document.getElementById("modalAjaxFooter").insertAdjacentHTML("afterbegin",options_installments);
						var cardType = "mastercard"; //"'.strtolower($vars->clientsdetails->cctype).'";
						var total = '.(int)($vars['total']*100).';
						console.log("cardType: "+cardType);
						console.log("total: "+total);
						if (cardType !== "undefined") {
							// Obtém opções de parcelamento
							try {
								EfiJs.CreditCard
									...
									.then(installments => {
										console.log("Parcelas", installments);
										let opcoes = "<option value=1>1 x de R$'.number_format($vars['total'],  2, ',', '.').' sem juros</option>";
        				    			for (let index = 1; index < installments.installments.length; index++) {
        				    			    opcoes += `<option value="${installments.installments[index].installment}">${installments.installments[index].installment} x de R$${installments.installments[index].currency} ${installments.installments[index].has_interest === false ? "sem juros" : ""}</option>`;
        				    			}
        				    			document.getElementById("installmentsSelect").innerHTML = opcoes;
								...
								console.log("Código 2: ", error.code);
								console.log("Nome 2: ", error.error);
								console.log("Mensagem 2: ", error.error_description);
							}
						}
					}
					
					document.getElementById("modalAjax").addEventListener("change", gefic_selcard)
					//document.getElementById("btnShowAttemptCaptureDialog").addEventListener("click", gefic_selcard);
					
					if(document.getElementById("installment_").value == "yes"){
						document.getElementById("installmentsSelect").addEventListener("change", gefic_selInstallments);
						document.getElementById("modalAjax").addEventListener("change", gefic_selInstallments)
					}
					
					function gefic_selInstallments(){
						var sel = document.getElementById("installmentsSelect");
						if(sel.value > 1){
							document.getElementById("cardcvv").value = sel.value;
						}
						else{
							document.getElementById("cardcvv").value = "1";
						}
						console.log(sel.value);
					}

					function gefic_selcard(){
						var selcard = document.getElementById("selectCard");
						console.log(selcard.value);
					}
