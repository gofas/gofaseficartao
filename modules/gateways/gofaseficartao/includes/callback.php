...
if( $_POST /*$_POST['paymentToken'] and $_POST['chargeReference'] and $_POST['chargeCode']*/ ){
	...
	if( (int)$invoice['invoiceid'] === (int)$_POST['chargeReference'] ){
		if( !function_exists('gefic_callback') ){
			function gefic_callback($charge_url,$postfields){
    		$curl = curl_init();
			$query = $charge_url;
		...
		$callback = json_decode(json_encode(gefic_callback($charge_url, $postfields)), true);
		...
		if( (int)$callback['success'] === 1 and $callback['data']['payment']['type'] === 'CREDIT_CARD' and ($callback['data']['payment']['status'] === 'CUSTOMER_PAID_BACK' or $callback['data']['payment']['status'] === 'BANK_PAID_BACK' or $callback['data']['payment']['status'] === 'PARTIALLY_REFUNDED') and $invoice['status'] === 'Paid'){
			...
			if($transIDend){
				...
				if( strpos( $transID_, 'geficc') !== false and strpos( $transID_, $api_mode) !== false ){
					$transID					= $transID_;
				}
				else {
					$transID				= false;
			...
			$AddTransaction = localAPI(
				...
				array(
					...
					'transid' => 'geficc-'.$callback['data']['payment']['charge']['code'].'-'.$api_mode.'-refund-'.$callback['data']['payment']['id'],
					'paymentmethod' => 'gofaseficartao',
					'date' => date("d/m/Y"),
	...
	if($params['log']){
		logModuleCall('gofaseficartao','receive_callback',array('module_version'=>'1.2.2','POST'=>$_POST,'invoice'=>$invoice,'postfields'=>$postfields),'', array( 'error'=>$error, 'callback'=>$callback,'$GetTransactions'=>$GetTransactions,'$AddTransaction'=>$AddTransaction,'$SendEmail'=>$SendEmail, '$UpdateInvoice'=>$UpdateInvoice ) );
	}
}